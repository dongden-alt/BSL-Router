            ...
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <div>
                    ...
                    <div style="display:flex; align-items:center; gap:8px; margin-top:4px;">
                        ...
                        <span style="font-size:11px; background:#f3f4f6; color:var(--text-muted); padding:2px 6px; border-radius:4px; font-weight:500;">${isOAuth ? 'OAuth' : 'Web'}</span>
...
function renderUsageTable() {
    ...
    html += `<div style="display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:16px;margin-bottom:18px;align-items:stretch;">
        <div style="background:var(--surface-color);border:1px solid var(--border-color);border-radius:18px;padding:16px;box-shadow:0 16px 40px rgba(15,23,42,.06);height:430px;display:flex;flex-direction:column;min-width:0;"><div style="font-size:13px;font-weight:900;margin-bottom:10px;">Provider Graph</div><div style="flex:1;min-height:0;">${usageSummaryState ? renderProviderGraphSVG(usageSummaryState.providers || []) : renderProviderGraphSVG(filteredData)}</div></div>
        <div style="background:var(--surface-color);border:1px solid var(--border-color);border-radius:18px;padding:16px;box-shadow:0 16px 40px rgba(15,23,42,.06);height:430px;display:flex;flex-direction:column;min-width:0;"><div style="font-size:13px;font-weight:900;margin-bottom:10px;text-align:center;">Provider Share</div><div style="flex:1;min-height:0;display:flex;align-items:stretch;justify-content:center;">${usageSummaryState ? renderProviderSharePie(usageSummaryState.providers || []) : renderProviderSharePie(filteredData)}</div></div>
    </div>

    <div style="background:var(--surface-color);border:1px solid var(--border-color);border-radius:18px;padding:16px;box-shadow:0 16px 40px rgba(15,23,42,.06);margin-bottom:18px;">${usageSummaryState ? renderConsumptionChartFromBuckets(usageSummaryState.buckets || []) : renderConsumptionChart(filteredData)}</div>
