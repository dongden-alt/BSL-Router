...
async def _bsl_agentic_max_dispatch(
    ...
    # -- Balanced orchestration loop (shares with Ultra, 2026-09-10) --
    # One client request -> N internal upstream calls (phase loop, lead + 1
    # parallel member on commit boundaries, cap 22) -> one final response.
    # Only engaged for coding-domain queries when agent_routes are configured;
    # chat-domain queries stay single-chain (fast mode).
    from app.middleware.blacksand_orchestrator import run_balanced
    from app.middleware.bsl_agentic_max_router import _build_effective_cfg

    _max_effective = _build_effective_cfg(config)
    if (max_decision.domain == "coding"
            and _max_effective.get("agent_routes")):

        async def execute_upstream(model_alias, messages, max_tokens, timeout):
            """One internal upstream call for an orchestration phase."""
            _chain = [model_alias]
            for _cell in (_max_effective.get("agent_routes") or {}).values():
                if isinstance(_cell, dict) and _cell.get("primary") == model_alias:
                    for _slot in ("fallback_1", "fallback_2"):
                        _v = _cell.get(_slot)
                        if isinstance(_v, str) and _v:
                            _chain.append(_v)
                    break
            _glf = _max_effective.get("global_last_fallback")
            if _glf:
                _chain.append(_glf)
            _seen_x = set()
            _chain = [e for e in _chain if e and not (e in _seen_x or _seen_x.add(e))]
            for _entry in _chain:
                ...
                if max_tokens:
                    _phase_body["max_tokens"] = max_tokens
                _phase_body["_bsl_original_model"] = "blacksand-agentic-max"
                _res = await _process_chat_completion(_phase_body, request=request)
                if isinstance(_res, JSONResponse) and _res.status_code in _RECOVERABLE:
                    print(
                        f"[BSLAgenticMax] internal '{_entry}' returned HTTP "
                        f"{_res.status_code}; advancing",
                        flush=True,
                    )
                    continue
        ...
        try:
            ...
            print(
                f"[BSLAgenticMax] orchestration loop failed: {_orch_exc} "
                f"-- falling back to single chain",
                flush=True,
            )
        if _orch is not None and _orch.final_text.strip():
            for _t in _orch.trace:
                print(
                    f"[BSLAgenticMax] phase {_t.phase_idx}/{_orch.phases_total} "
                    f"role={_t.sub_role} model={_t.model} "
                    f"boundary={_t.boundary or '-'}{' member' if _t.member else ''} "
                    f"{_t.tokens_in}in/{_t.tokens_out}out {_t.ms:.0f}ms",
                    flush=True,
            ...
            _routing_hdrs = {
                ...
                "X-BSL-Agentic-Max-Source": "balanced_orchestrator",
                "X-BSL-Agentic-Max-Selected": "blacksand-agentic-max",
                "X-BSL-Agentic-Max-Phases": str(_orch.phases_total),
                "X-BSL-Agentic-Max-Capped": str(_orch.capped),
                "X-BSL-Agentic-Max-Template": _orch.template_id,
                "X-BSL-Agentic-Max-Degraded": ",".join(_orch.degraded)[:500],
            }
            _cid = f"chatcmpl-max-{int(time.time() * 1000)}"
            if body.get("stream"):
                async def _max_sse():
                    _pieces = [
                        _orch.final_text[i : i + 200]
                ...
                _max_sid = next_stream_id()
                return StreamingResponse(
                    afz_guard(_max_sse(), _max_sid, protocol="openai", deadline_s=0),
                    media_type="text/event-stream",
                    headers=_routing_hdrs,
                )
            return JSONResponse(
...
class _ComboFallbackNeeded(Exception):
    ...
    """
    def __init__(self, status_code: int, err_text: str, retry_state: dict, backoff: float = 0.0):
        self.status_code = status_code
        self.err_text = err_text
        self.retry_state = retry_state
        # NEVER-STOP RETRY (2026-08-31): seconds the guarded catch handler
        # sleeps before recursing into the wrapped pass. 0 = plain advance.
...
def _raise_combo_wrap(model, config, request, retry_state, active_chain,
                      original_model, cache_bp, status_code, err_text, reason):
    """NEVER-STOP streaming wrap: exhaustion reached inside an SSE generator.

    Raises _ComboFallbackNeeded carrying a WRAPPED retry state (idx 0,
    pass_no+1, fresh deadline) plus the pass backoff, which the guarded
    catch handlers sleep before recursing (client disconnect during the
    sleep cancels it — the only permitted terminator). Returns without
    ...
    if _should_probe_stream_status(is_stream, active_chain, client_wants_gemini):
        try:
            ...
            if _probe_resp.status_code != 200:
                ...
                # NEVER-STOP RETRY (2026-08-31): probe-stage exhaustion (the
                # user-visible "[BSL Router] upstream error 429" force-stop)
                # wraps to pass 2+ instead of emitting terminal frames.
                if _combo_infinite_retry_enabled(config):
    ...
    if is_stream:
        ...
        async def raw_upstream(_attempt: int = 0):
            ...
            try:
                ...
                if not _emit.emitted:
                    # NEVER-STOP RETRY (2026-08-31): pre-emission exhaustion
                    # wraps to pass 2+ instead of a terminal error+DONE frame.
                    _raise_combo_wrap(
                        model, config, request, _retry_state, active_chain,
                        original_model, _cache_breakpoints, stats["status"],
                        stats["error"], reason="openai_transport_exhausted",
                    )
        ...
        async def raw_upstream_guarded():
            ...
            while True:
                ...
                try:
                    ...
                    if getattr(_cf_raw, "backoff", 0):
                        # NEVER-STOP wrap: sleep the pass backoff (client
                        # disconnect cancels — the only permitted terminator).
                        await asyncio.sleep(_cf_raw.backoff)
        ...
        if convert_egress:
            async def egress_stream(_attempt: int = 0):
                ...
                try:
                    ...
                    if not _emit.emitted:
                        # NEVER-STOP RETRY (2026-08-31): pre-emission exhaustion
                        # wraps to pass 2+ instead of terminal error frames.
                        _raise_combo_wrap(
                            model, config, request, _retry_state, active_chain,
                            original_model, _cache_breakpoints, stats["status"],
                            stats["error"], reason="anthropic_transport_exhausted",
                        )
            ...
            async def egress_stream_guarded():
                ...
                while True:
                    ...
                    try:
                        ...
                        if getattr(_cf_egr, "backoff", 0):
                            # NEVER-STOP wrap: sleep the pass backoff (client
                            # disconnect cancels — the only permitted terminator).
                            await asyncio.sleep(_cf_egr.backoff)
        ...
        if convert_gemini_egress:
            ...
            def _raise_gemini_combo_fallback(status_code: int, err_text: str, emit=None) -> None:
                ...
                if _next_state is None:
                    # NEVER-STOP RETRY (2026-08-31): chain exhausted pre-emission
                    # — wrap to pass 2+ instead of returning into the caller's
                    # terminal frame (the 429 force-stop path).
                    _raise_combo_wrap(
                        model, config, request, _retry_state, active_chain,
                        original_model, _cache_breakpoints, status_code,
                        err_text, reason="gemini_chain_exhausted",
                    )
                    return
                print(
                ...
                if _chain_budget_remaining() <= 0:
                    ...
                    # NEVER-STOP RETRY (2026-08-31): budget dead — wrap with a
                    # fresh deadline instead of silently refusing to advance.
                    _raise_combo_wrap(
                        model, config, request, _retry_state, active_chain,
                        original_model, _cache_breakpoints, status_code,
                        err_text, reason="gemini_budget_exhausted",
                    )
                else:
...
('"reasoning":"' in _c and '"reasoning":""' not in _c) or
                        ...
                        if _emit.may_fallback(stats.get("error") or "gemini_pre_content_stall"):
                            ...
                            if active_chain and _pre_next_idx < len(active_chain):
                                ...
                                if _chain_budget_remaining() <= 0:
                                    ...
                                    # NEVER-STOP RETRY (2026-08-31): budget dead —
                                    # wrap to a fresh pass instead of terminal frame.
                                    _raise_combo_wrap(
                                        model, config, request, _retry_state, active_chain,
                                        original_model, _cache_breakpoints, 504,
                                        stats.get("error") or "stream_stall",
                                        reason="gemini_stall_budget_exhausted",
                                    )
                                else:
                                ...
                                # pre-content — wrap to pass 2+ instead of the
                                # terminal stall frame below.
                                _raise_combo_wrap(
                                    model, config, request, _retry_state, active_chain,
                                    original_model, _cache_breakpoints, 504,
                                    stats.get("error") or "stream_stall",
                                    reason="gemini_stall_chain_exhausted",
                                )
            ...
            async def gemini_egress_stream_guarded():
                ...
                try:
                    ...
                    if getattr(_cf, "backoff", 0):
                        # NEVER-STOP wrap: sleep the pass backoff (client
                        # disconnect cancels — the only permitted terminator).
                        await asyncio.sleep(_cf.backoff)
            ...
            async def anthropic_to_openai_egress_stream_guarded():
                ...
                while True:
                    ...
                    try:
                        ...
                        if getattr(_cf_anthr, "backoff", 0):
                            # NEVER-STOP wrap: sleep the pass backoff (client
                            # disconnect cancels — the only permitted terminator).
                            await asyncio.sleep(_cf_anthr.backoff)
